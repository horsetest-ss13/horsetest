...
/datum/space_level/New(new_z, new_name, list/new_traits = list(), new_allocation_type = ALLOCATION_FREE)
	z_value = new_z
	name = new_name
	traits = new_traits
...
/datum/space_level/proc/is_box_free(low_x, low_y, high_x, high_y)
	// Check against existing virtual level boundaries
	...
	// Check against dummy reservations (used during async clearing)
	...
	// Check if turfs in this box have been modified from default state
	// Sample a few key turfs to avoid excessive checking
	var/sample_points = list(
		list(low_x, low_y),           // Bottom-left corner
		list(high_x, high_y),          // Top-right corner
		list(low_x, high_y),           // Top-left corner
		list(high_x, low_y),           // Bottom-right corner
		list((low_x + high_x) / 2, (low_y + high_y) / 2) // Center
	)

	for(var/list/point in sample_points)
		var/check_x = point[1]
		var/check_y = point[2]
		var/turf/T = locate(check_x, check_y, z_value)
		if(!T)
			continue
		// If the turf is not space, it's been modified - box is not free
		if(!istype(T, /turf/open/space))
			return FALSE
		// If the turf has contents (objects, mobs, etc), it's in use
